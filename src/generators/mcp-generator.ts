...
`;
  ...
  private generateReadme(
    ...
    // Check what patterns are actually included (from PRD OR defaults)
    // Error handling and validation are always included by default
    const hasErrorHandling = manifest.globalPatterns.some(p => p.pattern === 'error-handler');
    const hasValidation = manifest.globalPatterns.some(p => p.pattern === 'validation-schemas') ||
                          prd.tools.some(t => t.parameters.some(p => p.validation)) ||
                          true; // Always include - Zod validation is in every tool
    const hasLogging = prd.patterns.global.logging.enabled;
    const hasExternalDeps = prd.dependencies.external.some(d => d.enabled);
    const hasProgress = prd.tools.some(t => t.progressNotifications || t.longRunning);
...
`;
    ...
    // List enabled patterns with descriptions
    if (hasErrorHandling) {
      readme += `- **Structured Error Handling**: MCP ErrorCode for proper client handling\n`;
    }
    if (hasValidation) {
      readme += `- **Input Validation**: Zod schemas for runtime type safety\n`;
    }
    if (hasProgress) {
      readme += `- **Progress Notifications**: Long operations provide feedback\n`;
    }
    if (hasExternalDeps) {
      readme += `- **Graceful Degradation**: Fallback when external APIs unavailable\n`;
    }
    if (hasLogging) {
      readme += `- **Structured Logging**: Debug-friendly output via MCP protocol\n`;
    }
...
Most MCP implementations throw generic errors. This server uses McpError
with specific ErrorCode values so Claude Desktop can:
- Distinguish user errors (InvalidParams) from system errors (InternalError)
- Retry appropriately based on error type
...
`;
    ...
    if (hasValidation) {
      readme += `**Input Validation:**
Runtime validation catches malformed requests before they cause cryptic errors.
Zod schemas provide:
- Clear error messages identifying exactly what's wrong
- TypeScript type inference keeping code and schemas in sync
- Protection against injection and malformed data

`;
    }

    if (hasProgress) {
      readme += `**Progress Notifications:**
Long-running operations can timeout or appear frozen. Progress notifications:
- Keep users informed during multi-step operations
...
`;
    ...
    if (hasExternalDeps) {
      readme += `**Graceful Degradation:**
External APIs fail. This server includes fallback strategies so:
- Partial functionality continues when services are down
- Users get meaningful status rather than crashes
- Cached/default data provides continuity

`;
    }

    readme += `### Technical Stack
...
- Better IDE integration during development
- Type inference with Zod keeps schemas in sync

**Why Zod for validation?**
- Type inference prevents schema/type drift
